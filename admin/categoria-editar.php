...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$nombre || !$slug) {
        ...
        // Tipos corregidos: "ssssiii" (nombre, slug, desc, icono son strings;
        // orden, activo, id son enteros). La versión vieja tenía una "s" de más al final.
        $stmt_upd = $conexion->prepare("UPDATE categorias SET nombre=?, slug=?, descripcion=?, icono=?, orden_display=?, activo=? WHERE id=?");
        $stmt_upd->bind_param("ssssiii", $nombre, $slug, $desc, $icono, $orden, $activo, $id);
        if ($stmt_upd->execute()) {
            header('Location: /admin/categorias.php?mensaje=actualizada');
            exit;
        } else {
...
.check-label { display:flex; align-items:center; gap:8px; font-size:0.8rem; color:var(--marlene); cursor:pointer; }
.btn-guardar {
    ...
    <form method="POST">
        <div class="form-card">
            ...
            <div class="form-group">
                <label>Nombre *</label>
                <input type="text" name="nombre" id="nombre" required value="<?= htmlspecialchars($cat['nombre']) ?>">
            </div>
            <div class="form-group">
                <label>Slug (URL) *</label>
                <input type="text" name="slug" id="slug" required value="<?= htmlspecialchars($cat['slug']) ?>">
                <div class="slug-preview">marlene.store/catalogo?cat=<span id="slug-text"><?= htmlspecialchars($cat['slug']) ?></span></div>
            </div>
            <div class="form-group">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="icono" value="<?= htmlspecialchars($cat['icono'] ?? '') ?>" placeholder="🎒" maxlength="10">
                </div>
                <div class="form-group">
                    <label>Orden de visualización</label>
                    <input type="number" name="orden_display" value="<?= (int)$cat['orden_display'] ?>" min="0">
                </div>
            </div>
            <div class="form-group">
                <label class="check-label">
                    <input type="checkbox" name="activo" style="width:auto;" <?= $cat['activo'] ? 'checked' : '' ?>>
