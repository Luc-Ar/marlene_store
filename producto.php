...
$titulo      = htmlspecialchars($producto['nombre']) . ' — Marlene STORE';
...
.prod-breadcrumb a { color: #999; text-decoration: none; }
.prod-breadcrumb a:hover { color: var(--color-marlene, #5C3D3E); }
.prod-subcategoria {
    font-size: 11px;
    font-weight: 700;
    color: #C9A96E;
...
.prod-acciones { display: flex; gap: 12px; flex-wrap: wrap; }
.btn-carrito {
    flex: 1;
    padding: 16px 24px;
    background: #5C3D3E;
...
.btn-carrito:hover  { background: #C9A96E; }
...
.prod-meta span { font-size: 12px; color: #999; }
.prod-meta strong { color: #5C3D3E; }
.relacionados {
    max-width: 1100px;
    margin: 60px auto;
    padding: 0 24px;
...
.rel-card:hover { transform: translateY(-4px); box-shadow: 0 10px 30px rgba(0,0,0,0.08); }
.rel-card img {
    width: 100%;
    aspect-ratio: 1;
    object-fit: contain;
...
.rel-card-nombre { font-weight: 600; font-size: 0.9rem; margin-bottom: 4px; }
.rel-card-precio { color: #5C3D3E; font-weight: 700; font-size: 1rem; }
.page-breadcrumb {
    max-width: 1100px;
    margin: 24px auto 0;
    padding: 0 24px;
...
.page-breadcrumb a:hover { color: #5C3D3E; }
@media (max-width: 768px) {
    .prod-detalle { grid-template-columns: 1fr; gap: 30px; }
